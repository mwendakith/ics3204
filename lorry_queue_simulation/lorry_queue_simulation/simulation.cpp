...
void Arrival::run_task(CClientDC& dc) { // uses Truck
    ...
    for (auto& p : p_sd_->pro_units) {
        if (p->is_free()) {
            p->add(p_truck_);
			move_along_line(dc, p_truck_->getGraphic(), p->get_x(), p->get_y());
...
void Completed::run_task(CClientDC& dc) {
	...
	move_along_line(dc, p_truck->getGraphic(), -50, p_processing_unit_->get_y());
    ...
    if (!p_sd_->truck_queue.empty()) {
		...
		move_along_line(dc, p_sd_->truck_queue.front()->getGraphic(), p_processing_unit_->get_x(), p_processing_unit_->get_y());
